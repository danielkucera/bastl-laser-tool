#!/usr/bin/env python 
"""
Modified by Milan B. 2017 www.bastl.sk
Modified by Jindra Fucik 2017 www.fucik.name
...
;--- end of default header
""",
'footer': """;--- start of default footer
G1 X0 Y0
M18
""",
...
class laser_gcode(inkex.Effect):
    ...
    def export_gcode(self,gcode):
        ...
        if self.options.laser_mode == "rep":
          ...
          f.write(self.options.laser_off_command + " S0" + "\n" + self.header + self.move_cmd+ " F" + self.options.travel_speed + "\n" + gcode + self.footer)
        f.close()
    ...
    def __init__(self):
        ...
        self.OptionParser.add_option("",   "--laser-mode",                      action="store", type="string",          dest="laser_mode",                          default="rep",                          help="Laser Mode")
        ...
        self.OptionParser.add_option("",   "--alt-intensity-command",           action="store", type="string",          dest="intensity_command",                   default="",                             help="Laser intensity command")
        self.OptionParser.add_option("",   "--speed-modulation",                action="store", type="inkbool",         dest="speed_modulation",                    default=False,                          help="Use speed to modulate intensity")
        self.OptionParser.add_option("",   "--acc-control",                     action="store", type="inkbool",         dest="acc_control",                         default=True,                           help="Use speed to modulate intensity")
        self.OptionParser.add_option("",   "--repetier-steps",                  action="store", type="int",             dest="repetier_steps",                      default=80,                             help="Use speed to modulate intensity")
        self.OptionParser.add_option("",   "--repetier-acc",                    action="store", type="int",             dest="repetier_acc",                        default=950,                            help="Use speed to modulate intensity")
        ...
        self.OptionParser.add_option("",   "--prologue",                        action="store", type="string",          dest="prologue",                            default="",                             help="Additional initialization g-code")
        self.OptionParser.add_option("",   "--z-base",                          action="store", type="float",           dest="z_base",                              default=0.00,                           help="Initial Z-Level")
        self.OptionParser.add_option("",   "--x-offset",                        action="store", type="float",           dest="x_offset",                            default=0.00,                           help="Initial X-Offset")
        self.OptionParser.add_option("",   "--y-offset",                        action="store", type="float",           dest="y_offset",                            default=0.00,                           help="Initial Y-Offset")
    ...
    def check_dir(self):
        ...
        if (os.path.isdir(self.options.directory)):
            if (os.path.isfile(self.options.directory+'header')):
                ...
                self.header = f.read()
                self.header += "; --- end of custom header\n"
                f.close()
            else:
                if self.options.laser_mode =="rep":
                  self.header = defaults['rheader']
                else:
                  self.header = defaults['header']
            if (os.path.isfile(self.options.directory+'footer')):
                f = open(self.options.directory+'footer','r')
                self.footer = "; --- begin of custom footer\n"
                self.footer += f.read()
                f.close()
            else:
                if self.options.laser_mode =="rep":
                  self.footer = defaults['rfooter']
                else:
...
###
################################################################################
    def generate_gcode(self, curve, of, layer, depth):
        tool = self.tools
        ...
        def c(c):
            ...
            for i in range(6):
                if c[i]!=None:
                    if i==0:
                      r += s[i] + ("%f" % (round(c[i]+self.options.x_offset,4))).rstrip('0')
                    elif i==1:
                      r += s[i] + ("%f" % (round(c[i]+self.options.y_offset,4))).rstrip('0')
                    else:
	...
	ofi = of
        ofs = of
        if self.options.speed_modulation:
            ofi = 1.0
            spd = self.options.laser_speed / ( 0.05 if of < 0.05 else of)
        else:
            ofi = of
            spd = self.options.laser_speed


#        lg, f =  'G00', "F%f"%tool['penetration feed']
#        penetration_feed = "F%s"%tool['penetration feed'] 
#        spd = self.options.laser_speed / ofs
        lg, f =  'G00', "F%f"%spd
        penetration_feed = "F%s"%spd

        g = ''

        accset = ''
        accreset = ''
        if self.options.laser_mode == "rep" and self.options.acc_control:
            cm = self.options.repetier_acc
            while cm >= 0:
              mins = 60.0 * cm * math.sqrt( 2.0 / ( self.options.repetier_steps * cm ))
              if mins < spd:
                if cm < self.options.repetier_acc:
                  if cm <= 0:
                    cm = 1
                  accset = "M201 X" + str(cm)+" Y" + str(cm) + "\nM202 X" + str(cm) + " Y" + str(cm) + "\n"
                  accreset = "M201 X" + str(self.options.repetier_acc)+" Y" + str(self.options.repetier_acc) + "\nM202 X" + str(self.options.repetier_acc) + " Y" + str(self.options.repetier_acc) + "\n"
                break
              cm -= 5
...
#                  g += "G1"
                if self.options.laser_mode == "rep":
                  if self.options.intensity_command != "":
                    g += self.move_cmd + c(si[0]) + "\n" + tool['gcode before path A1'] + str(int(float(self.options.laser_power)*ofi))
                  else:
                    g += self.move_cmd + c(si[0]) + "\n" + tool['gcode before path A2']
                  g += accset + "\n"
                else:
                  g += self.move_cmd + c(si[0]) + "\n" + tool['gcode before path A'] + str(int(float(self.options.laser_power)*ofi))
                  if self.options.intensity_command != "":
                    g += tool['gcode before path A1'] + str(int(float(self.options.laser_power)*ofi))
                  g += tool['gcode before path B'] +"\n"
                lg = 'G00'
            elif s[1] == 'end':
            ...
            if layer in paths :
                ...
                for path in paths[layer] :
                    ...
                    if "style" in path.keys() : 
                        mystyle=path.get("style")
                        mystylelist=mystyle.split(";")
                        for onestyl in mystylelist :
                            if onestyl.startswith("opacity:"):
                               of = float(onestyl.split(":")[1])
                               print_("opacity=",of)
                    ...
                    if path.get("dxfpoint") == "1":
                        ...
                        gcode += self.generate_gcode(curve, of, layer, 0)
                        p = []    
...
################################################################################
    def orientation(self, layer=None) :
        ...
        #myunit = 96      # myunit is transformation of document units to "in", default is "px", default resolution is 96 dpi
        #if self.getDocumentUnit() == "mm" :
        #    myunit = 25.4
        #if self.getDocumentUnit() == "in" :
        #    myunit = 1
        #if self.getDocumentUnit() == "px" :
        #    myunit = 96
        #if self.getDocumentUnit() == "cm" :
        #    myunit = 2.54
        #if self.getDocumentUnit() == "pt" :
        #    myunit = 72
